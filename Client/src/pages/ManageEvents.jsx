...
import { getEvents, updateEventsStatus, reset } from '../redux/slices/eventCalendarSlice';
...
const ManageEvents = () => {
  ...
  const handleSave = () => {
    try {
      dispatch(getEvents()); // Refetch events to get the latest updates
      setShowModal(false);
      setSelectedEvent(null);
  ...
  const handleToggleActive = (event) => {
    ...
    dispatch(updateEventsStatus({ id: event._id, eventData: updatedEvent }))
      .unwrap()
      .then(() => {
        dispatch(getEvents()); // Refetch after update
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4 flex justify-end items-center">
        <button onClick={() => setShowModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-md">
          Add New Event
        </button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-300">
          ...
          <tbody>
            {eventsArray.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((event, index) => (
              <AdminEventCard
                key={event._id || index}
                event={event}
